...
internal static class CTPManager
{
    ...
    public static CTPMainWindow ShowCTP()
    {
        ...
        if (!_ctpInstances.TryGetValue(currentWorkbook, out var ctp) || ctp == null)
        {
            // 为当前工作簿创建新的 TaskPane
            _wpfControl = new CTPMainWindow();
            ctp = GetCTP(_wpfControl, "ExcelSharp");
    ...
    public static void DeleteCTP()
    {
        ...
        if (currentWorkbook != null && _ctpInstances.TryGetValue(currentWorkbook, out var ctp) && ctp != null)
        {
            ctp.Delete();
    ...
    private static void SetTaskPaneWidthToThirdOfExcelWindow(CustomTaskPane taskPane)
    {
        ...
        var excelHwnd = new IntPtr(excelApp.Hwnd);
        GetWindowRect(excelHwnd, out var excelRect);
        var excelWindowWidthPixels = excelRect.Width;
